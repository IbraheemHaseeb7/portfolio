...
export default function ContactForm({
  ...
  return (
    <form className="w-1/2 grid grid-cols-2 justify-items-center gap-5 ml-[4rem]">
      {contacts.map((data) => {
        if (data.name === "description") {
          return (
            <div className={data.classes}>
              <span className="text-creamy ml-2 capitalize">{data.name}</span>
              <textarea
      ...
      <button className="border-2 border-creamy col-[1/3] mt-5 px-[0.75rem] py-[0.25rem] text-creamy hover:text-pinky transition-all ease duration-[0.25s]">
        submit
      </button>
    </form>
  );
